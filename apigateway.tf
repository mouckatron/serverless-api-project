...
resource "aws_api_gateway_integration" "lambdas" {
  ...
  type                    = "AWS_PROXY"
  # add ${stageVariables.environment} into the end of the string
  uri = join("", [trimsuffix(aws_lambda_function.lambda[count.index].invoke_arn, "/invocations"), ":$${stageVariables.environment}/invocations"])
}
